...
class Mode:
    """
    Container for possible modes for an iPerf benchmark.
    """
    POD_TO_POD = "PodToPod"
...
def on_create(namespace, name, spec, **kwargs):
    ...
    # If the mode indicates a service, create one
    if spec["mode"] == Mode.POD_TO_SERVICE:
...
def on_server_phase_changed(namespace, name, spec, status, **kwargs):
    """
    Executes when the server phase of an iperf benchmark changes.
    """
    ...
    if not server_phase or server_phase != util.DeploymentPhase.AVAILABLE:
        return
    # Work out the server IP to use
    if spec["mode"] == Mode.POD_TO_POD:
        server_ip = status.get("server", {}).get("podIP")
    else:
        server_ip = status.get("server", {}).get("serviceIP")
    if not server_ip:
        raise kopf.TemporaryError("Unable to determine server IP")
    # Create the job for the client
    job = iperf.from_template("client-job.yaml", name = name, spec = spec, server_ip = server_ip)
    kopf.adopt(job)
...
def on_output_changed(status, patch, **kwargs):
    ...
    for output_line in reversed(status["output"].splitlines()):
        match = re.search(r"\d+\.\d+ [KMGT]bits/sec", output_line)
...
@iperf.on_subresource_event("service")
@util.suppress_not_found()
def on_service_event(type, spec, namespace, labels, **kwargs):
    """
    Executes when an event occurs for a service that is a subresource of an iperf benchmark.
    """
    if type != "DELETED":
        cluster_ip = spec.get("clusterIP")
        if cluster_ip:
            patch = dict(status = dict(server = { "serviceIP": cluster_ip }))
            iperf.apply_patch(namespace, labels["app.kubernetes.io/instance"], patch)
